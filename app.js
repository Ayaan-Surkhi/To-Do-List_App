...
if(localStorage.length !== 0){
   ...
   previousValues.forEach(value => {
    ...
    newTodo.innerHTML =  `<p class = 'point'>\u2022</p> <div class = 'new-todo' id = 'new-todo-${todos.length}'>${value.todo}</div> <i class="far fa-check-square check" id = 'check-${todos.length}'></i> <i class="fas fa-undo undo" id = 'undo-${todos.length}'></i> <i class="fas fa-edit edit" id = 'edit-${todos.length}'></i> <i class="fas fa-trash-alt delete" id = 'delete-${todos.length}'></i>`;
...
addTodo.addEventListener('change', (e) => {   
    ...
    localStorage.setItem('todos', JSON.stringify(todos));
    newTodo.innerHTML =  `<p class = 'point'>\u2022</p> <div class = 'new-todo' id = 'new-todo-${todos.length}'>${e.target.value}</div> <i class="far fa-check-square check" id = 'check-${todos.length}'></i> <i class="fas fa-undo undo" id = 'undo-${todos.length}'></i> <i class="fas fa-edit edit" id = 'edit-${todos.length}'></i> <i class="fas fa-trash-alt delete" id = 'delete-${todos.length}'></i>`;
    console.log(todos);
    list.appendChild(newTodo);
    todoInterface.scrollTop = todoInterface.scrollHeight;
